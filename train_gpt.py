...
class Hyperparameters:
    ...
    mlp_mults = [int(x) for x in os.environ.get(
        "MLP_MULTS", "3,3,3,3,3,3,3,4,4,4,4"
    ).split(",")]
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mlp_mults = mlp_mults
        self.qo_bank = nn.Parameter(torch.empty(2 * num_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * num_layers, kv_dim, model_dim))
        self._mlp_up_list = nn.ParameterList([
            nn.Parameter(torch.empty(int(mlp_mults[i] * model_dim), model_dim))
            for i in range(num_layers)
        ])
        self._mlp_down_list = nn.ParameterList([
            nn.Parameter(torch.empty(model_dim, int(mlp_mults[i] * model_dim)))
            for i in range(num_layers)
        ])
        # tied layer setup
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(n):
            ...
            nn.init.orthogonal_(self._mlp_up_list[i].data, gain=1.0)    # MLP up
            nn.init.zeros_(self._mlp_down_list[i].data)                  # MLP down (zero init)
            # Scale proj layers (out_proj and mlp_down are "proj" layers)
            self.qo_bank.data[n + i].mul_(proj_scale)
            self._mlp_down_list[i].data.mul_(proj_scale)
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if bi == self.num_layers - 1:
                x_tied, _ = self.blocks[self.tied_layer_source](
                    x, x0,
...
def _normalize_sd_for_export(sd: dict) -> dict:
    out = {}
    for k, v in sd.items():
        if k.startswith("_mlp_up_list."):
            i = int(k.split(".")[1])
            out[f"mlp_up_bank.{i}"] = v
        elif k.startswith("_mlp_down_list."):
            i = int(k.split(".")[1])
            out[f"mlp_down_bank.{i}"] = v
        else:
            out[k] = v
    return out
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    ...
    for name, tensor in sd.items():
        if name == "qo_bank":
            ...
            i = int(name.split(".")[1])
            out[f"blocks.{i}.mlp.fc.weight"] = tensor
        elif name.startswith("mlp_down_bank."):
            i = int(name.split(".")[1])
            out[f"blocks.{i}.mlp.proj.weight"] = tensor
        else:
            out[name] = tensor
    return out
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    n = num_layers
    for i in range(n):
        up_key = f"_mlp_up_list.{i}"
        down_key = f"_mlp_down_list.{i}"
        if up_key not in template_sd:
            raise KeyError(f"template_sd missing required key: {up_key}")
        if down_key not in template_sd:
            raise KeyError(f"template_sd missing required key: {down_key}")
    ...
    for i, t in enumerate(up_slices):
        if t is not None:
            out[f"_mlp_up_list.{i}"] = t.to(dtype=template_sd[f"_mlp_up_list.{i}"].dtype)
    for i, t in enumerate(down_slices):
        if t is not None:
            out[f"_mlp_down_list.{i}"] = t.to(dtype=template_sd[f"_mlp_down_list.{i}"].dtype)
    for name, tensor in sd.items():
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 mlp_mults: list[int], tie_embeddings, logit_softcap, rope_base, qk_gain_init,
        ...
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        self.blocks = nn.ModuleList([
            _HessianBlock(model_dim, num_heads, num_kv_heads, mlp_mults[i], rope_base, qk_gain_init,
...
def main() -> None:
    ...
    args = Hyperparameters()
    assert len(args.mlp_mults) == args.num_layers, \
        f"mlp_mults length {len(args.mlp_mults)} != num_layers {args.num_layers}"
    ...
    for p in base_model._mlp_up_list:
        p.data = p.data.float()
    for p in base_model._mlp_down_list:
        p.data = p.data.float()
    ...
    matrix_params = (
        ...
        + list(base_model._mlp_up_list)
        + list(base_model._mlp_down_list)
    )
    ...
    scalar_params.append(base_model.tie_gate)
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        decay_mul = remaining_ms / max(warmdown_ms, 1e-9) if remaining_ms <= warmdown_ms else 1.0
        return decay_mul
    ...
    sd_cpu = _normalize_sd_for_export(sd_cpu)
    ...
    hessian_model = _HessianGPT(
        ...
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mults=args.mlp_mults,
    ...
    target_mb = float(os.environ.get("TARGET_MB", "15.0"))
    ...
    for p in eval_model._mlp_up_list:
        p.data = p.data.float()
    for p in eval_model._mlp_down_list:
        p.data = p.data.float()
